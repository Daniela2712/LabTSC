...
if {$compile_on || [batch_mode] == 0} {
  ...
  #vlog -sv -timescale "1ns/1ns" -work work       -f sources.txt
   vlog -sv -timescale "1ps/1ps" -cover bcesft -work work       -f sources.txt
                                       #branch condition statemant
}

# Load project
 eval vsim -novopt -quiet -nocoverage +notimingchecks +nowarnTSCALE -sva -sv_seed $1 +NUMBEROFTRANSACTION=$2 top
# eval vsim -novopt -quiet -coverage -notogglevlogints +notimingchecks +nowarnTSCALE +TESTNAME=$1 -sva top
#eval vsim -novopt -quiet -coverage +code=bcesft +notimingchecks +nowarnTSCALE -sva -sv_seed $1 +NUMBEROFTRANSACTION=$2  top
